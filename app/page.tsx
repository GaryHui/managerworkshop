...
export default function Home() {
  ...
  const [audit, setAudit] = useState({ url: "", platform: "淘宝", keyword: "", impressions: "12000", clicks: "228", carts: "31", orders: "8", price: "49.9", competitor: "45.9" });
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav>{["总览", "店铺分工", "商品中心", "流量诊断", "推广任务", "价格与利润"].map((item, i) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}><span className="nav-icon">{["⌂", "店", "品", "诊", "✓", "¥"][i]}</span>{item}{item === "推广任务" && <b>10</b>}</button>)}</nav>
