...
export default function Home() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav>{["总览", "店铺分工", "商品中心", "推广任务", "价格与利润"].map((item, i) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}><span className="nav-icon">{["⌂", "店", "品", "✓", "¥"][i]}</span>{item}{item === "推广任务" && <b>10</b>}</button>)}</nav>
